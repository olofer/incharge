...
const fragmentShaderSource = `#version 300 es
  ...
  vec2[3] pos_vel_acc(vec2 osc, vec2 rho, float omega) {
    float cs = osc.x;
    float sn = osc.y;
    vec2[3] rva;
    rva[0] = vec2(cs * rho.x, sn * rho.y);
    rva[1] = omega * vec2(-1.0 * rho.x * sn, rho.y * cs);
    rva[2] = -1.0 * omega * omega * vec2(rho.x * cs, rho.y * sn);
    return rva;
  }
  ...
  void main() {
    ...
    vec2[3] rva = pos_vel_acc(osc_tau, rho, OMEGA);
    vec2 rsrc = uv - rva[0];
    vec2 beta = rva[1] / c;
    ...
    if (u_style == 0) {
      color = vec3(0.0, dot(A, A), Q);
    } else if (u_style == 1) {
      vec2 E = electric_field(1.0 + Rmod, BETA, c, rsrc, beta, rva[2]);
